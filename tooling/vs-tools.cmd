...
for /f "usebackq tokens=*" %%i in (`call %VSWHERE% -prerelease -latest -requires Microsoft.VisualStudio.Workload.NativeDesktop -version "[16.0,18.0)" -find VC\Auxiliary\Build\vcvars64.bat`) do (
    call "%%i" && exit /b 0
)
